...
def read_amn(amn_content: str) -> np.ndarray:
    ...
    return np.transpose(
        amn_data[:, :, :, 3] + 1j * amn_data[:, :, :, 4], axes=(0, 2, 1)
    )
...
def cwf_window_function(
    ...
    return (
        fermi_dist_func(mu_min - energy, sigma_min)
        + fermi_dist_func(energy - mu_max, sigma_max)
        - 1.0
        + delta
    )
...
def fit_cwf_parameters_raw(
    ...
    projectability = np.real(
        np.diagonal(amn @ amn.transpose(0, 2, 1).conjugate(), axis1=1, axis2=2)
    )

    energies_flat = energies.reshape(-1)
    projectability_flat = projectability.reshape(-1)

    def fit_function(x_values, mu_min, mu_max, sigma_min, sigma_max):
        return cwf_window_function(
            x_values, mu_min, mu_max, sigma_min, sigma_max, delta
        )

    initial_values = (
        float(np.min(energies_flat) - 10.0),
        float(np.max(energies_flat) + 10.0),
        0.0,
        10.0,
    )
    lower_bounds = (-np.inf, -np.inf, 0.0, 0.0)
    upper_bounds = (np.inf, np.inf, 100.0, 100.0)
    ...
    mu_min_fit, mu_max_fit, sigma_min_fit, sigma_max_fit = optimized
...
def fit_cwf_parameters_from_contents(
    ...
    if energies.shape[:2] != amn.shape[:2]:
        raise ValueError(
            "Inconsistent eig/amn contents: "
            f"energies shape={energies.shape}, amn shape={amn.shape}"
        )
