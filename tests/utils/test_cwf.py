...
def test_fit_cwf_parameters_from_contents():
    ...
    eig_content = "\n".join(f"{band_index} 1 {energy:.16f}" for band_index, energy in enumerate(energies, start=1))
    ...
    # lmfit can converge to slightly different values from scipy curve_fit
    # while still reproducing the target window well, so keep the tolerance modest.
    assert abs(parameters["cwf_mu_min"] - mu_min) < 5e-2
    assert abs(parameters["cwf_mu_max"] - (mu_max - 3.0 * sigma_max)) < 5e-2
    assert abs(parameters["cwf_sigma_min"] - sigma_min) < 5e-2
    assert abs(parameters["cwf_sigma_max"] - sigma_max) < 5e-2
