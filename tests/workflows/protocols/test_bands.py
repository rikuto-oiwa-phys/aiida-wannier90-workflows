...
"""Tests for the ``Wannier90BandsWorkChain.get_builder_from_protocol`` method."""
import pytest
...
def test_get_default_protocol():
    """Test ``Wannier90BandsWorkChain.get_default_protocol``."""
    assert Wannier90BandsWorkChain.get_default_protocol() == "moderate"
...
def test_closest_wannier_inputs(generate_builder_inputs):
    """Test configuring ``Wannier90BandsWorkChain`` for Closest Wannier."""
    builder = Wannier90BandsWorkChain.get_builder_from_protocol(
        **generate_builder_inputs(),
        projection_type=WannierProjectionType.ATOMIC_PROJECTORS_QE,
        print_summary=False,
